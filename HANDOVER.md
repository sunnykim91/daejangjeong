...
- **지형 타일셋**: `assets/art/gfx/Overworld.png`(CC0 Zelda-like, OpenGameArt). total.html `TILE`/`TSRC`(지형별 [col,row] 16px)로 `renderMapBg`가 32px 블릿. 오프라인 시 절차 폴백. ⚠️ 캐릭터/성/UI는 아직 절차생성 — 미통일. **디자이너 에셋 오면 교체 예정**(사용자 방침: 프로토 플레이스홀더).
...
5. 사운드·세이브·튜토리얼 없음. 성능 O(n²) 현 규모 무난.
